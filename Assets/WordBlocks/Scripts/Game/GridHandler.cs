...
namespace BizzyBeeGames.WordBlocks
{
	public class GridHandler : UIMonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
	{
		...
		[SerializeField] private GridLetterTile letterTilePrefab = null;
		...
		{
			if (numTilesMoving > 0 || activeLevelSaveData == null)
			{
				return;
			}
		...
		{
			if (numTilesMoving > 0 || activePointerId != eventData.pointerId || activeLevelSaveData == null)
			{
				return;
			}
		...
		{
			// Set the save data reference to null, mouse events will now be ignored
