...
namespace BizzyBeeGames.WordBlocks
{
	...
	{
		void Instance_OnSwitchingScreens(string arg1, string arg2)
		{
		}
		...
		[SerializeField] private WordListHandler		wordListHandler			= null;
		[SerializeField] private SelectedWordHandler	selectedWordHandler		= null;
		[SerializeField] private GridHandler			gridHandler				= null;
		[SerializeField] private LevelCompleteHandler	levelCompleteHandler	= null;
		...
		[SerializeField] private ExtraWordsButton		extraWordsButton		= null;

		[Space]

		[Header("Anim Settings - Grid Letters")]
		[SerializeField] private float letterAnimDuration	= 0;
		[SerializeField] private float delayBetweenLetters	= 0;
		...
		public void SetupGame(LevelData levelData, LevelSaveData levelSaveData)
		{
			...
			levelHintText.text		= "HINT: " + levelData.Hint;

			// Make sure the selected word is not visible
			selectedWordHandler.Clear();
		...
		{
			bool adShown = GameManager.Instance.TryShowInterstitialAd(null);

			if (!adShown)
			{
				// We dont want to play the button click if an ad is shown because the click sound happens after the ad finishes
		...
		{
			// If we are mocing away from the game screen make sure and BoardCreator workers are stopped
		...
		{
			// In some cases it's possible for a word to be selected when the player is not on the game screen (They select a word then click the back
			// button). So don't actually select a word if the current screen is ot the game screen.
		...
		private void HandleWordInLevelSelected(string word, List<int[]> gridCellPositions)
		{
			...
			if (IsLevelComplete())
			{
				gridHandler.NotifyLevelCompleted();
